...
exports.postExpenseDetails = async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "Invalid details", success: false });
    } else {
      ...
      User.findOne({ where: { id: req.user.id } }, { transaction: t })
        ...
        .then(async () => {
          ...
          res
            .status(201)
            .json({ addedExpense: data, message: "expense added" });
        })
        .catch(async (err) => {
          await t.rollback();
...
exports.getExpenseDetails = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ err: "Something went wrong", success: false });
  }
};
...
exports.downloadexpense = async (req, res) => {
  try {
    ...
    console.log(fileUrl);
    const id = uuid.v4();
    const urladdedtotable = await Downloads.create({
      id,
      fileUrl,
      userId: req.user.id,
    });
...
exports.getUrlTable = async (req, res) => {
  try {
    const response = await Downloads.findAll({
      where: { userId: req.user.id },
    });
