...
function getExpenses(page) {
  ...
  axios
    .get(
      `http://localhost:3000/expense/get-expense?page=${page}&itemPerPage=${ItemPerPage}`,
      { headers: { Authorization: token } }
    )
    .then((response) => {
...
window.addEventListener("load", async () => {
  ...
  if (isPremium) {
    ...
    document.getElementById("premiumUser").innerHTML +=
      "You are a premium user  <button onclick=showPremiumFeatures() >Show Leaderboard</button>";
    document.getElementById("downloadexpense").innerHTML +=
      "<button onclick=download()>Download File</button>";
    document.getElementById("urlTable").innerHTML +=
      "<button onclick=showUrlTable()>Recent Downloads</button>";
  }
  showPagination(response.data);
  if (response.status === 200) {
    for (var i = 0; i < response.data.AllExpenses.length; i++) {
...
function showUrlTable() {
  axios
    .get(`http://localhost:3000/expense/urlTable`, {
      headers: { Authorization: token },
    })
    .then((response) => {
      if (response.status === 201) {
...
document.getElementById("rzp-button1").onclick = async function (e) {
  ...
  var options = {
    ...
    handler: async function (response) {
      await axios.post(
        "http://localhost:3000/purchase/updatetransactionstatusSuccess",
        {
  ...
  rzp1.on("payment.failed", async function (response) {
    console.log(response);
    await axios.post(
      "http://localhost:3000/purchase/updatetransactionstatusFail",
      {
        order_id: options.order_id,
      },
      { headers: { Authorization: token } }
